<template>
  <div class="margin-t-20 fujian flex flex-wrap" v-if="inited">
    <div v-for="(item ,index) in  structure" :key="index"
         :class="item.fileMaximum > 1 ? 'item multiple': 'item single'">
      <div class="title text-center">{{item.subCategoryName}}</div>
      <div class="box">
        <div class="uploadzone" v-if="viewFlag==0">
          <input v-if="item.fileOpration!=1" ref="inputFile" :multiple="item.fileMaximum > 1"
                 @change="fnChange($event,item,index)" type="file" class="uploadBtn">
          <div class="iconfont icontianjia"></div>
          <p>{{item.fileMaximum > 1 ? '上传多个附件':'上传单个附件'}}</p>
        </div>

        <div class="file" :class="item.fileMaximum == 1 ? 'singleFile' : ''" v-if="item.fileList.length>0"
             v-for="(file,idxFile) in item.fileList" :key="idxFile">
          <div class="img"><img :src="file.fileId.endsWith('.pdf') ? pdfIcon  : file.thumbnailUrl" alt=""></div>
          <div class="actions"><span v-if="item.fileOpration!=1 && viewFlag==0 " @click="fnRemove(item,idxFile)"
                                     class="iconfont iconlajitong margin-r-20"></span><span @click="fnDownload(file)"
                                                                                            class="iconfont icondr"></span>
          </div>
        </div>
      </div>
    </div>
...
import 'yxfeui/iview/src/styles/components/.fujian.less'
