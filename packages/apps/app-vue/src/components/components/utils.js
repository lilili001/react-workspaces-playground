...
const componentObj = {
    ...
    group:generateGroupComponent,
    tabbar:generateTabbarComponent
}

function generateTabbarComponent(h, formData = {}, obj, vm) {
    let components = []
    const key = obj.key? obj.key : ''
    if (obj.children) {
        components = obj.children.map(tabpane => {
            var childComponents = tabpane.children.map(item=>{
                let component;
...
function generateGroupComponent(h,formData = {} , obj , vm) {
    const key = obj.key? obj.key : ''
    let children = [];
    if (obj.children) {
        children = obj.children.map(item => {
            let component;
                ...
                component =   generateFormItemComponent(h,item,component);
            if(item['props'] && item['props']['span']){
                component =  generateColComponent(h,item.props,component)
            }
...
function generateInputComponent(h, formData = {}, obj, vm) {
    ...
    if(key.indexOf('.')>0){
        ...
        var {lastLevalObj} = transformStr2Obj(formData,key);
        tKey = lastLevalObj[lastKey];
    }else{
        tKey = formData[key]
    }
    return h('Input', {
        ...
        on: {
            ...
            input(val) {
                if (key) {
                    simpleKey ? formData[key] = val : lastLevalObj[lastKey] = val
                }
            }
        },
        slot: obj.slot
...
function generateFormItemComponent(h, obj, component) {
    var a = obj.type=='group' || obj.type == 'tabbar' ? 'div' : 'FormItem';
    return h(a, {
        class: obj.className,
        props: {
            label: obj.label,
