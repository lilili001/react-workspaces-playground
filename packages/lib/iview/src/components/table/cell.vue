<template>
    <div :class="classes" ref="cell">
        ...
        <template v-if="renderType === 'expand' && !row._disableExpand">
            <div v-if="!row._noExpand" class="ivu-table-cell flex-box" @click="toggleExpand(row._noExpand)" style="cursor: pointer;margin-left:-16px">
                <div class="expand-icon-box flex-box flex-center">
                    <Icon :type="expanded ? 'ios-remove' : 'ios-add'" />
                </div>
                <div>{{row[column.key]}}</div>
            </div>
            <div class="ivu-table-cell flex-box" v-else-if="row._hasTag">
                <span class="margin-r-8">{{row[column.key]}}</span>
                <div class="table-tag">{{row[column.tagKey]}}</div>
            </div>
            <div class="ivu-table-cell flex-box" v-else>{{row[column.key]}}</div>
