<template>
    <div :class="classes" :style="listStyle">
        <div :class="prefixCls + '-header'">
            ...
            <span :class="prefixCls + '-header-count'" v-html="count" style="font-size:12px"></span>
        </div>
        <div :class="bodyClasses">
            ...
            <ul :class="prefixCls + '-content'">
                <li
                    ...
                    <Checkbox :value="isCheck(item)" v-if="!item.isTiTle" :disabled="item.disabled"></Checkbox>
    ...
    export default {
        ...
        computed: {
            ...
            count () {
                ...
                return (validKeysCount > 0 ? `<span style="color:#1890FF">${validKeysCount}</span>/` : '') + `${totalCount}`;
            },
            checkedAll () {
        ...
        methods: {
            itemClasses (item) {
                return [
                    ...
                    {
                        [`${this.prefixCls}-content-item-title`] : item.isTiTle
                    }
                ];
            },
            showLabel (item) {
            ...
            toggleSelectAll (status) {
                const keys = status ?
                        this.filterData.filter(data =>!data.isTiTle&&!data.disabled || !data.isTiTle&&this.checkedKeys.indexOf(data.key) > -1).map(data => data.key) :
                        this.filterData.filter(data =>!data.isTiTle&&data.disabled && this.checkedKeys.indexOf(data.key) > -1).map(data => data.key);
