        ...
        methods: {
            ...
            splitData (init = false) {
                ...
                if(this.multidimensional){
                    ...
                    this.rightData = [];
                    if (this.targetKeys.length > 0){
                        this.targetKeys.forEach((targetObj)=>{
                            let innerArr =[];
                            const   filteredData = this.leftData.filter((data,index)=>{
                                let _index = index;
                                if(data.key === targetObj.parentKey){
                                    let _data = data;
                                    targetObj.config.forEach((innerObj)=>{
                                        const innerFilterData = _data.config.filter((item,indexKey)=>{
                                            if(item.key === innerObj){
                                                this.leftData[_index].config.splice(indexKey,1);
                                                return true;
                                            }
                                            return false;
                                        });        
                                        // console.log(innerFilterData,33333);
                                        if(innerFilterData && innerFilterData.length>0) innerArr.push(innerFilterData[0]);
                                    });
                                    return true;
                                }else{
                                    return false;
                            ...
                            if (filteredData && filteredData.length > 0) {
                                filteredData[0].config = innerArr;
                                this.rightData.push(filteredData[0]);
                            }
                            console.log(this.rightData,222);
                        });
                    }
                }
                if (init) {
